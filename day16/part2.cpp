  ...
  for (int v : adj[curr]) {
    if (!visited[v]) {
      ...
      if (time > 0) {
        ...
        max = std::max(
            max,
            get_max_flow(adj, cost, rates, nonzero_rates, visited, v, time) + flow
        );
      }
    }
  }
...
int solve(
    const std::vector<std::vector<int>> &adj, const std::vector<std::vector<int>> &cost,
    const std::vector<int> &rates, const std::vector<int> &nonzero_rates, int start, int time
) {
  int len = nonzero_rates.size(), max = 0;
  for (int i = 0; i < (1 << (len - 2)); i++) {
    std::vector<bool> me(len, false), elephant(len, false);
    for (int j = 0; j < len; j++) {
      me[j] = (i >> j) & 1;
      elephant[j] = !me[j];
    }
    ...
    max = std::max(
      max, (
          get_max_flow(adj, cost, rates, nonzero_rates, me, start, time) +
          get_max_flow(adj, cost, rates, nonzero_rates, elephant, start, time)
      )
    );
  }
  return max;
...
int main(int argc, char *argv[]) {
  if (argc > 0) {
    ...
    if (file.is_open()) {
      ...
      std::cout << (
          solve(modified_adj, cost, rates, nonzero_rates, nonzero_rates.size() - 1, time)
      ) << std::endl;
      file.close();
    }
  }
