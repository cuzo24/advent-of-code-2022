...
struct Monkey {
  std::deque<long long int> items;
  Operation op;
  int operand;
  int on_true;
  int on_false;
  int test;
  long long int business_level = 0;
};
...
long long int get_worry_level(
    Operation op, int operand, long long int old_val, long long int mod
) {
  return (
      op == SUM ?
        old_val + (operand == - 1 ? old_val : operand) :
        old_val * (operand == - 1 ? old_val : operand)
  ) % mod;
}
      ...
      for (int i = 0; i < rounds; i++) {
        for (int j = 0; j < monkeys.size(); j++) {
          while (!monkeys[j].items.empty()) {
            ...
            item = get_worry_level(monkeys[j].op, monkeys[j].operand, item, mod);
